import { apiPost, processSource } from "@/lib/api-client";
...
export async function generateStudySummary(studySetId: string) {
  return apiPost<{ id: string; content: string }>(
    `/study-sets/${studySetId}/summary`,
  );
}

export async function generateFlashcards(studySetId: string, count = 12) {
  return apiPost<{
    deckId: string;
...
export async function sendStudyChat(conversationId: string, message: string) {
  return apiPost<{ reply: string; sources: unknown[] }>(
